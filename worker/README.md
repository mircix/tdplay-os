...
3. *Settings → Variables and Secrets*: add **secrets** `IG_APP_ID`, `IG_APP_SECRET`, `ADMIN_KEY` (any long random string) and a plain variable **`ALLOWED_ORIGINS`** = `https://mircix.github.io,http://localhost:8787`.
...
**D. Connect** — open `https://<worker>/auth/callback`'s sibling once: `https://<worker>/auth/start?key=<ADMIN_KEY>` → log in with @mitch_tdp → "Connected".
